---
name: excavator-architecture-analyzer
description: |
...
Before writing the script, create its input JSON file. First resolve the project's data directory once and reuse `$DATA_DIR` for every path below:
...
1. Write the JSON array to the `intermediate/layers.json` file inside the project's data directory — `$DATA_DIR/intermediate/layers.json` (`.excavator/`). Use the exact output path given in your dispatch prompt if one was provided.
