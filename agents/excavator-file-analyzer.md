...
Create the input file with the batch data. **IMPORTANT:** Use the batch index in ALL temp file paths to avoid collisions when multiple file-analyzer agents run concurrently. First resolve the project's data directory once and reuse `$DATA_DIR` for every path below: `DATA_DIR="$PROJECT_ROOT/.excavator"`.
...
**Step B — Decide split.**
- If `nodeCount ≤ 60` AND `edgeCount ≤ 120`: write ONE file to `$DATA_DIR/intermediate/batch-<batchIndex>.json` (the data directory — `.excavator/`). Done. Skip to Step F.
...
Write part `k` (1-indexed) to `$DATA_DIR/intermediate/batch-<batchIndex>-part-<k>.json`. Each part is a valid GraphFragment: `{ "nodes": [...], "edges": [...] }`.
