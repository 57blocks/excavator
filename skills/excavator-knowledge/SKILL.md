...
1. Determine the target directory:
   ...
   - **Resolve the data directory `$DATA_DIR`** once, and reuse it for every read and write below: `DATA_DIR="<TARGET_DIR>/.excavator"`.
...
2. Run the format detection script bundled with this skill:
   ...
   - If successful, proceed. The script writes `scan-manifest.json` to `$DATA_DIR/intermediate/`
...
3. For each batch, dispatch an `article-analyzer` subagent with:
   ...
   - The intermediate directory path: `$INTERMEDIATE_DIR = $DATA_DIR/intermediate`
...
5. Clean up intermediate files. Resolve `$DATA_DIR` into a shell variable and guard it so an empty or unresolved path can never expand to `rm -rf /intermediate` (deleting from the filesystem root):
   ```bash
   TARGET_DIR="<TARGET_DIR>"
   DATA_DIR="$TARGET_DIR/.excavator"
   if [ -n "$TARGET_DIR" ] && [ -d "$DATA_DIR/intermediate" ]; then
