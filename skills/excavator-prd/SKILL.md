...
**Translate codes — never print raw values.** Numeric enum/status codes, boolean flags, and database field values are meaningless to the reader. Always render them as their human meaning: write the status label (e.g. "In Progress / Done / Blocked"), not `status=2`; "a Jira-type detail line", not `content_type=2`; "already pushed, not resent", not `update_sent=1`. Resolve every code by reading the enum/constant definition in source (do not guess the mapping). When the code↔meaning mapping itself is worth recording, put it in a **field-dictionary table** (columns: value | meaning), never inline in prose. A bare code, flag, or `field=value` left in the running text is a defect — the reader should never have to look up a number. (These examples are English because this skill file is English; the generated PRD itself follows `--language`.)
...
3. **Enum check.** Grep your own draft for raw code values left in prose (`status=`, `content_type=`, bare `field=number`, untranslated flags). If any survive outside a field-dictionary table or `<details>` block, translate them to their human meaning before finishing.
4. **Save.** Offer to write the PRD to the project (default `docs/PRD.md`, or a path the user gives; a scoped PRD may use `docs/PRD-<scope>.md`). Suggest committing it for the team.
