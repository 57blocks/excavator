...
7. **Decision logic (full analysis only — reached only when `$ANALYSIS_MODE` is `full`):**
   ...
   **`full-semantic-isolation` scope note.** "Full analysis (all phases)" (and, below, `FULL_UPDATE`) no longer means "run Phase 1 through Phase 7 below" — it means **Phase F**, the section after Phase 0. Phase 1 through Phase 7 below are unchanged and still govern every `PARTIAL_UPDATE` / `ARCHITECTURE_UPDATE` / `SKIP` destination (they already skip Phase 1 for those), plus the `--review` review-only path. Phase F reuses the exact same deterministic fact build Lazy mode uses instead of letting file-analyzer author `knowledge-graph.json`'s nodes/edges/layers directly — see Phase F's own header for why.
   ...
   **Review-only path:** Copy the existing `knowledge-graph.json` to `$DATA_DIR/intermediate/assembled-graph.json`, then jump directly to Phase 6 step 3.
   ...
   | `FULL_UPDATE` | Run **Phase F** below (not the legacy Phase 1-7 pipeline — see the scope note above). Do not patch fingerprints or metadata from the incremental helper; Phase F's own Phase F1 (re)writes them. |
