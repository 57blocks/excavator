...
`full` 模式在模型运行结束后 SHALL 检查：运行结果没有报告错误；没有被拒绝的权限请求；没有失败的子 agent；产物记录的 commit 等于运行前的 HEAD；产物记录的模型等于本次请求的模型 ID，而不是 `unknown`。任一项不满足时，运行 SHALL 以加载或产物检查失败的退出码结束（模型运行本身出错或触及花费上限时以运行失败的退出码结束），摘要写明原因；已有产物 SHALL 保留。

#### Scenario: 子 agent 失败
- **WHEN** 运行结果报告至少一个子 agent 失败
- **THEN** 运行以加载或产物检查失败的退出码结束，摘要写明失败的子 agent 数
...
除配置错误外（配置错误只输出到标准错误，见「运行参数不全时先失败」），每次运行 SHALL 在输出目录写出运行摘要，至少包含：镜像记录的 Excavator commit 与 Claude Code 版本、目标仓库 HEAD、运行模式、模型 ID、每一项检查的结论与原因、按类别（输入、缓存写入、缓存读取、输出）的 token 数、估算花费，以及编造数与未核验数。缓存读取 token 为 0 时摘要 SHALL 带警告。退出码 SHALL 固定为：0 成功或跳过，2 配置错误，3 运行失败，4 加载或产物检查失败，5 编造超阈值；每次运行 SHALL 恰好对应其中一个。
...
镜像 SHALL 提供不调用模型、不需要凭证的自检：版本与标签一致、插件清单校验通过、`python` 命令可用、在临时生成的小仓库上 Lazy 能产出知识图谱、挂载的仓库属主与容器用户不同时 git 仍可用。发布流程 SHALL 在打 tag 时运行仓库测试门、构建 amd64 与 arm64 两种架构并在两者上运行自检，全部通过后才以不可覆盖的标签 `<插件版本>-<短 commit>` 推送到镜像仓库。镜像仓库参数未配置时，发布流程 SHALL 只构建与自检，并明确报告跳过了推送。

#### Scenario: 未配置镜像仓库
- **WHEN** 在没有配置镜像仓库参数的情况下触发发布流程
- **THEN** 两种架构都完成构建与自检，流程报告推送被跳过，而不是失败或静默成功
