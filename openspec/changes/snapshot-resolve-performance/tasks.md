...
- [ ] 3.1 `docs/mcp.md`「Performance and host timeouts」一节改写：删掉「每次调用 O(文件数) 的子进程读取」和「wcp 上 75–115 秒」这类已不成立的描述，写入验收实测的解析耗时与每次调用耗时，并按新数字重新给出超时建议。验证：文中数字与 4.3 的实测一致。
