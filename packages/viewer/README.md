...
Standalone read-only viewer for [Excavator](https://github.com/Jingqi-57blocks/excavator) knowledge graphs. Opens the full interactive dashboard for a graph that was already generated with `/excavator` — no Claude Code, no LLM, no API key. Only Node.js (>= 18) is required.
...
```bash
pnpm --filter excavator-viewer pack:release
gh release upload <tag> packages/viewer/excavator-viewer-*.tgz
```
