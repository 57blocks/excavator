...
`);
  ...
  // Identity (design D2): trait, enum and anonymous-class methods were not
  // walked at all, so every declaration inside them was invisible.
  describe("extractStructure - traits, enums, anonymous classes, owner", () => {
    it("extracts trait methods with the trait as owner", () => {
      const { tree, parser, root } = parse(`<?php
trait Greets {
...
`);
      ...
      expect(result.classes.map((c) => c.name)).toEqual(["Greets"]);
      expect(result.classes[0].methods).toEqual(["hello"]);
    ...
    it("extracts enum methods with the enum as owner", () => {
      const { tree, parser, root } = parse(`<?php
enum Status: string {
    case Draft = 'draft';
...
`);
      ...
      expect(result.classes.map((c) => c.name)).toEqual(["Status"]);
      expect(result.functions.map((f) => [f.name, f.owner])).toEqual([
        ["label", "Status"],
      ]);
    ...
    it("names an anonymous class by its line and owns its methods", () => {
      const { tree, parser, root } = parse(`<?php
$x = new class {
...
`);
      ...
      expect(result.classes.map((c) => c.name)).toEqual(["anon@2"]);
      expect(result.functions.map((f) => [f.name, f.owner])).toEqual([
        ["run", "anon@2"],
      ]);
    ...
    it("keeps two same-named methods apart by their declaring class", () => {
      const { tree, parser, root } = parse(`<?php
class A {
...
`);
      const result = extractor.extractStructure(root);
      const saves = result.functions.filter((f) => f.name === "save");

      expect(saves).toHaveLength(3);
      // The top-level function has no owner at all; the two methods name theirs.
      expect(saves.filter((f) => f.owner === undefined)).toHaveLength(1);
      expect(saves.map((f) => f.owner).filter(Boolean).sort()).toEqual(["A", "B"]);
