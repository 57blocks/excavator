...
`);
  ...
  // Identity (design D2): two same-named methods in one file are only
  // distinguishable by their declaring type.
  describe("extractStructure - owner", () => {
    it("reports the declaring type as owner for methods and constructors", () => {
      const { tree, parser, root } = parse(`namespace N
{
    public class Repo
    {
        public Repo() {}
...
`);
      const result = extractor.extractStructure(root);
      const owned = result.functions.map((f) => `${f.owner}.${f.name}`).sort();

      expect(owned).toEqual(["Other.Get", "Repo.Get", "Repo.Repo"]);

      tree.delete();
      parser.delete();
    });

    it("keeps two overloads of one method in the same owner group", () => {
      const { tree, parser, root } = parse(`public class Overloads
{
    public string Get(string key) { return key; }
...
`);
      ...
      expect(gets.every((f) => f.owner === "Overloads")).toBe(true);
      // Same path, owner and name: only the line numbers separate them.
      expect(gets[0].lineRange[0]).not.toBe(gets[1].lineRange[0]);
